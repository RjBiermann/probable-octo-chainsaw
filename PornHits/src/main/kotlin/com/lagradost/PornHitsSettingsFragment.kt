...
class PornHitsSettingsFragment : DialogFragment() {
    ...
    private fun resolveAttr(theme: android.content.res.Resources.Theme, tv: TypedValue, customAttr: String, fallbackAttr: Int, context: Context): Int {
        ...
        return if (customId != 0 && theme.resolveAttribute(customId, tv, true)) {
            ...
            Log.w(TAG, "Failed to resolve color attribute: $customAttr (fallback: $fallbackAttr)")
            0
        }
    }
    ...
    private fun createSettingsView(context: Context): View {
        ...
        val urlInputLayout = TextInputLayout(context, null, com.google.android.material.R.attr.textInputOutlinedStyle).apply {
            ...
            hint = "Paste URL from pornhits.com"
        ...
        sectionsRecyclerView = RecyclerView(context).apply {
            ...
            this.adapter = this@PornHitsSettingsFragment.adapter
        ...
        urlInput.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable?) {
                ...
                when (val result = PornHitsUrlValidator.validate(text)) {
                    ...
                    is ValidationResult.InvalidDomain -> {
                        statusText.text = "URL must be from pornhits.com"
                        labelInputLayout.visibility = View.GONE
                        addButton.isEnabled = false
                    }
                    is ValidationResult.InvalidPath -> {
                        statusText.text = "Invalid URL (use category, pornstar, site, or search pages)"
                        labelInputLayout.visibility = View.GONE
                        addButton.isEnabled = false
                    }
                }
        ...
        addButton.setOnClickListener {
            val result = PornHitsUrlValidator.validate(urlInput.text.toString())
            if (result is ValidationResult.Valid) {
                ...
                if (currentPages.none { it.path == newPage.path }) {
                    ...
                    if (saveCustomPages(context, currentPages)) {
                        ...
                        Log.e(TAG, "Failed to save custom pages after adding new page")
                        statusText.text = "Failed to save. Storage may be full."
                        // Revert the addition since save failed
                        currentPages.remove(newPage)
                    }
                    adapter.submitList(currentPages)
                    updateEmptyState()
