...
class NepornSettingsFragment : DialogFragment() {
    ...
    private fun createSettingsView(context: Context): View {
        ...
        sectionsRecyclerView = RecyclerView(context).apply {
            ...
            this.adapter = this@NepornSettingsFragment.adapter
        ...
        urlInput.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable?) {
                ...
                when (val result = NepornUrlValidator.validate(text)) {
                    ...
                    is ValidationResult.InvalidDomain -> {
                        statusText.text = "URL must be from neporn.com"
                        labelInputLayout.visibility = View.GONE
                        addButton.isEnabled = false
                    }
                    is ValidationResult.InvalidPath -> {
                        statusText.text = "Invalid URL (use category, tag, model, or search pages)"
                        labelInputLayout.visibility = View.GONE
                        addButton.isEnabled = false
                    }
                }
        ...
        addButton.setOnClickListener {
            val result = NepornUrlValidator.validate(urlInput.text.toString())
    ...
    private fun loadCustomPages(@Suppress("UNUSED_PARAMETER") context: Context): List<CustomPage> {
        return try {
            ...
            Log.e("NepornSettings", "Failed to load custom pages (${e.javaClass.simpleName})", e)
            emptyList()
        }
    }
    ...
    private fun saveCustomPages(@Suppress("UNUSED_PARAMETER") context: Context, pages: List<CustomPage>): Boolean {
        return try {
            ...
            Log.e("NepornSettings", "Failed to save custom pages (${e.javaClass.simpleName})", e)
            false
        }
    }
